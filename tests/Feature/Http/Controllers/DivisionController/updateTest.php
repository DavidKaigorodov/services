...
describe('DivisionController@update', function () {

    test('Администратор может обновить подразделение', function () {
        $this->actingAs($this->admin)
            ->put(route('divisions.update', $this->division->id), $this->attrs)
    ...
    test('Обновление подразделения с невалидными данными', function () {
        $rules = [
        ...
        foreach ($rules as $field => $fieldRules) {
            ...
            foreach ($invalidValues as $invalid) {
                ...
                $this->actingAs($this->admin)
                    ->put(route('divisions.update', $this->division->id), $badAttrs);
    ...
    test('Работник не может обновить подразделение', function () {
        $this->actingAs($this->division_worker)
            ->put(route('divisions.update', $this->division->id), $this->attrs)
            ->assertStatus(403);

        $this->assertDatabaseMissing('main__divisions', [
            'id' => $this->division->id,
            'name' => $this->attrs['name'],
        ]);
    });

    test('Администратор подразделения не может обновить подразделение', function () {
        $this->actingAs($this->division_admin)
            ->put(route('divisions.update', $this->division->id), $this->attrs)
            ->assertStatus(403);
    });

    test('Гость не может обновить подразделение', function () {
        $this->put(route('divisions.update', $this->division->id), $this->attrs)
